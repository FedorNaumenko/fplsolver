...
// Shared with transferSuggestions — same minutes + fixture difficulty model
...
/**
 * Greedy multi-transfer planner.
 *
 * Each iteration finds the single best available transfer from the current
 * (possibly already modified) squad, applies it virtually, then repeats.
 * This won't always find the global optimum but is fast and practical.
 *
 * @param freeTransfers  How many transfers are free this week.
 *                       Pass a large number (e.g. 99) for wildcard mode.
 * @param numTransfers   Maximum number of transfers to plan.
 */
export function planMultipleTransfers(
  ...
  for (let i = 0; i < numTransfers; i++) {
    ...
    for (const playerOut of currentSquad) {
      ...
      const xPtsOut = calcXPts(playerOut, fixtures, currentGameweek, numGameweeks);
      ...
      const best = allPlayers
        ...
        .map(p => ({ player: p, ep: calcXPts(p, fixtures, currentGameweek, numGameweeks) }))
      ...
      if (gain > bestGain) {
        ...
        bestTransfer = {
          ...
          costDiff: best.player.now_cost - playerOut.now_cost,
          xPtsGain: gain,
        };
      }
    }

    // Stop early if no beneficial transfer exists
    if (!bestTransfer || bestGain <= 0) break;

    // Apply the transfer virtually so the next iteration sees the updated squad
    currentSquad = currentSquad
      .filter(p => p.id !== bestTransfer!.playerOut.id)
      .concat([bestTransfer.playerIn]);
    currentBank =
      currentBank + bestTransfer.playerOut.now_cost - bestTransfer.playerIn.now_cost;
  ...
  const totalXPtsGain =
    Math.round(plannedTransfers.reduce((sum, t) => sum + t.xPtsGain, 0) * 10) / 10;
