...
function PitchPlayer({
  ...
  return (
    <button
      onClick={onClick}
      className="flex flex-col items-center gap-1 w-16 group focus:outline-none"
      title={`${player.web_name} — tap for details`}
    >
      <div className="relative">
        <div
          className={`w-11 h-11 rounded-full flex items-center justify-center text-white text-[11px] font-bold shadow-md transition-transform group-hover:scale-110 group-active:scale-95 ${POSITION_BG[player.element_type]}`}
        >
      ...
      <div className="bg-white/90 backdrop-blur-sm rounded px-1 py-0.5 shadow text-center w-full group-hover:bg-white transition-colors">
...
function BenchPlayer({ player, onClick }: { player: Player; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="flex flex-col items-center gap-1 w-14 group focus:outline-none"
      title={`${player.web_name} — tap for details`}
    >
      <div className="relative">
        <div
          className={`w-9 h-9 rounded-full flex items-center justify-center text-white text-[10px] font-bold shadow opacity-80 transition-transform group-hover:scale-110 group-hover:opacity-100 group-active:scale-95 ${POSITION_BG[player.element_type]}`}
        >
          {player.web_name.slice(0, 3).toUpperCase()}
        </div>
        <StatusDot status={player.status} />
    ...
    <>
      <div className="rounded-xl shadow-lg overflow-hidden">
          ...
          <div className="flex justify-center gap-4 sm:gap-8">
            {bench.map((player, i) => (
              <div key={player.id} className="flex flex-col items-center gap-1">
                ...
                <BenchPlayer player={player} onClick={() => setSelectedPlayer(player)} />
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-gray-900 px-4 py-2 flex gap-4 flex-wrap">
          ...
          <div className="ml-auto text-[10px] text-gray-500 italic">Tap a player for details</div>
        </div>
      </div>
